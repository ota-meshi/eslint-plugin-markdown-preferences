...
let insertText = `\n${renderRulesTableContent(
  2,
  (name) =>
    `https://ota-meshi.github.io/eslint-plugin-markdown-preferences/rules/${name}.html`,
)}\n`;
if (isWin) {
...
fs.writeFileSync(
  docsReadmeFilePath,
  newReadme
    .replace("# eslint-plugin-markdown-preferences\n", "# Introduction\n")
    .replace(
    ...
    .replace(
      /\(https:\/\/ota-meshi.github.io\/eslint-plugin-markdown-preferences/gu,
      "(.",
    )
    .replace(/\n{3,}/gu, "\n\n"),
  "utf8",
